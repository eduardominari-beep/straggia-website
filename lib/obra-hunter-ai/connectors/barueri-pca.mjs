...
const SOURCE_URL = "https://www.barueri.sp.gov.br/Transparencia/downloads/Planejamento/PCA_2026_-_Cronograma_de_Contrata%C3%A7%C3%B5es_OBRAS.pdf?ent=1&np=1";
...
export async function runBarueriPcaConnector({ runId, allowFixtureFallback = false } = {}) {
  ...
  try {
    ...
    // Parser de PDF ainda depende de OCR/text extractor no runtime.
    throw new Error(`live_pdf_parser_not_available raw_file=${rawFile}`);
  } catch (liveError) {
    logger.warn("Barueri live collection failed", { error: String(liveError), sourceUrl: SOURCE_URL });
